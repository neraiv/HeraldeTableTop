...
function displayItemDescription(inventroyItem, x, y) {
    ...
    if(item.spells && item.spells.length >0){
        ...
        item.spells.forEach(property => {
            ...
            if (property.availableClasses && property.availableClasses.length > 0) {
                const matchedStatTypes = Object.keys(classType).filter(key => 
                    property.availableClasses.includes(classType[key])
                );
                text += "<br>Avaliable Classes: " + matchedStatTypes.join(" / ");
            }     
            if (property.baseStatType && property.baseStatType.length > 0) {
                const matchedStatTypes = Object.keys(statType).filter(key => 
                    property.baseStatType.includes(statType[key])
                );
                text += "<br>Empowered With: " + matchedStatTypes.join(" / ");
            }            
            if (property.damageTypes && property.damageTypes.length > 0) {
                const matchedDamageTypes = Object.keys(damageType).filter(key => 
                    property.damageTypes.includes(damageType[key])
                );
                text += "<br>Damage Type: " + matchedDamageTypes.join(" / ");
            }
            if(property.description){
                text += "<br>Description: " + property.description;
            }
            if(property.spellPattern){
                text += "<br>Pattern: " + Object.keys(patterns).find(key => patterns[key] === property.spellPattern.pattern);
            }
            if(property.baseDamage){
                text += "<br>Dice: " + property.baseDamage;
            }
            if(property.castTime){
                text += "<br>Cast Time: " + property.castTime;
            }
